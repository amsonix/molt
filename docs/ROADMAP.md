...
| Fog/FogAssets class-name randomization (auto-extraction target hardening) | ✅ Shipped (1.2.0): decrypt class names derived from seed (`fogClassName` / `fogAssetsClassName`), different per build; keep rules generated against the exact applicationId prefix |
